...
class Vertical_FL_Train:
    ...
    def run(self, args, learning_rates, batch_size):
        ...
        #p2vfl
        epsilons = [] #* total_parties   #,1,1,1,1,1]
                    ...
                    for bidx in range(len(batch_idxs_list)):
                        ...
                        if len(organization_outputs_for_val) >= 2:
                            for organization_idx in range(1, total_parties):
                                val_noise, val_grad_noise = add_Gaussian_noise(organization_outputs_for_val[organization_idx], epsilons[organization_idx], delta, sensitivity)
                
                                organization_outputs_for_val_cat = torch.cat((organization_outputs_for_val_cat, val_noise), 1).float()
        ...
        for batch_idxs in batch_idxs_list:
            with torch.no_grad():
                ...
                org_out_cat = None
                for organization_idx in range(total_parties):  # total_parties should match the training configuration
                    client_output = organization_models[organization_idx](
                        X_val_vertical_FL[organization_idx][batch_idxs].to(device)
                    )
                    if org_out_cat is None:
                        org_out_cat = client_output
                    else:
                        org_out_cat = torch.cat((org_out_cat, client_output), dim=1)
        ...
        return train_loss_array, val_loss_array, train_auc_array, val_auc_array, contribution_term_array, privacy_term_array, rewards_array, utility_array,epsilons_array, time_taken