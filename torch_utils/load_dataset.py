...
#     # Determine slice widths
#     base_width = 32 // k
#     remainder = 32 % k
#     widths = [base_width + (1 if i < remainder else 0) for i in range(k)]
    
#     # Allocate slices as PyTorch tensors
#     image_parts = [
#         torch.zeros((N, 3, 32, widths[i]), dtype=images.dtype, device=images.device)
#         for i in range(k)
#     ]
...
#         for i in range(k):
#             end_col = current_col + widths[i]
#             image_parts[i][n] = images[n, :, :, current_col:end_col]
#             current_col = end_col

#     return image_parts 
# def vertical_split_images(images, k):
  
#     N = images.shape[0]  # Number of images in the batch
#     # Determine widths for each split
#     base_width = 32 // k
#     remainder = 32 % k
#     widths = [base_width + (1 if i < remainder else 0) for i in range(k)]
    
#     # Initialize empty arrays for each part
#     image_parts = [np.zeros((N, 3, 32, widths[i]), dtype=images.dtype) for i in range(k)]
    
#     # For each image, compute the slice boundaries independently.
#     for n in range(N):
#         current_col = 0  # Reset for each image
#         for i in range(k):
#             end_col = current_col + widths[i]
#             if images.shape[1] == 3:  # channels first
#                 image_parts[i][n] = images[n, :, :, current_col:end_col]
#             else:  # channels last
#                 image_parts[i][n] = images[n, current_col:end_col, :, :]
#             current_col = end_col

#     # Convert each part to a PyTorch tensor
#     return [torch.from_numpy(part).float() for part in image_parts]
