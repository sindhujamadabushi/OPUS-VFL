from torch_vertical_FL_train_incentive import Vertical_FL_Train
import argparse
import time
import sys
import random
...
# Hyperparameters
# hyper_parameters = {
#     'learning_rate_top_model': 0.00001, 
#     'learning_rate_organization_model': 0.000005, 
#     'batch_size': 512
# }
dataset = 'MNIST'
default_organization_num = '7'
...
def objective(trial):
    ...
    learning_rate_top_model = trial.suggest_float("learning_rate_top_model", 1e-6, 1e-3, log=True)
    learning_rate_organization_model = trial.suggest_float("learning_rate_organization_model", 1e-6, 1e-3, log=True)
    batch_size = trial.suggest_categorical("batch_size", [128, 256, 512, 1024])
    
    learning_rates = [learning_rate_top_model, learning_rate_organization_model]

    # Update args with suggested parameters
    args.batch_size = batch_size

    # Initialize active clients (can be modified based on your experiment)
    active_clients = [True] * int(args.organization_num)
    ...
    for i in range(num_iterations):
        vfl_model = Vertical_FL_Train(active_clients=active_clients)
        _, _, _, _, test_acc = vfl_model.run(args, learning_rates, batch_size)
        test_acc_sum += test_acc

    # Calculate average test accuracy
    test_acc_avg = test_acc_sum / num_iterations

    # Optuna minimizes by default; use negative accuracy for maximization
    return test_acc_avg
