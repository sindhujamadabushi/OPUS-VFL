...
from torch_vertical_FL_train_incentive_fia_CIFAR10 import Vertical_FL_Train
import optuna
import argparse
import time
import sys
...
dataset = 'CIFAR10'
default_organization_num = 5  # put one less than the number of clients you like
...
def run_base_model(args):
    ...
    # Total parties is organization_num + 1 (active party + other organizations)
    total_parties = args.organization_num + 1

    # Initialize accumulator arrays with matching shapes
    contribution_term_array_sum = np.zeros((args.epochs, total_parties))
    privacy_term_array_sum = np.zeros((args.epochs, total_parties))
    rewards_array_sum = np.zeros((args.epochs, total_parties))
    utility_array_sum = np.zeros((args.epochs, total_parties))
    epsilons_array_sum = np.zeros((args.epochs, total_parties))	

    for i in range(num_iterations):
        vfl_model = Vertical_FL_Train(active_clients=active_clients)
        print("Iteration: ", i)
        train_loss_array, val_loss_array, train_auc_array, val_auc_array, contribution_term_array, privacy_term_array, rewards_array, utility_array, epsilons_array, time_taken = vfl_model.run(args, learning_rates, batch_size)
        ...
        # Now this addition should work if contribution_term_array has shape (args.epochs, total_parties)
