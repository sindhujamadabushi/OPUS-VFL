    ...
    def run(self, args, learning_rates, batch_size):
        ...
        if data_type == 'original':
            if args.dname == 'CIFAR10':
                ...
                samples_per_class = 3000  # change this number as needed
        ...
        print("organization_num befoe organization_output_dim: ", organization_num)
        ...
        print("top model input dimension: ", sum(organization_output_dim))
        ...
        # for idx in poison_source_indices:
        ...
        for i in range(epochs):
            ...
            for bidx in range(len(batch_idxs_list)):
                ...
                # batch_num_poisoned = np.intersect1d(np.array(batch_idxs_list), selected_poison_indices.cpu().numpy())
                ...
                # print("organization_num before combining inputs: ", )
            ...
            print("tt for 1 e: ", time.time()-st)   
        # batch_size = 64
        # N = X_train_vertical_FL[0].shape[0]  # total number of training images
        ...
        #     batch_first = X_train_vertical_FL[0][i:i+batch_size].to(device)
        #     batch_second = X_train_vertical_FL[1][i:i+batch_size].to(device)
        #     batch_labels = y_train[i:i+batch_size].to(device)
            
        #     batch_second.requires_grad = True

        #     with torch.no_grad():
        #         batch_first_features = organization_models[0](batch_first)
            
        #     batch_second_features = organization_models[1](batch_second)
            
        #     batch_combined_features = torch.cat((batch_first_features, batch_second_features), dim=1)
            
        #     batch_scores = top_model(batch_combined_features)
        ...
        #     batch_saliency = batch_second.grad.abs()
        #     batch_saliency, _ = torch.max(batch_saliency, dim=1)  # shape: (batch_size, H, W)
            
        #     all_saliency_maps.append(batch_saliency.cpu().detach())

        # saliency_maps = torch.cat(all_saliency_maps, dim=0)
        # saliency_maps_np = saliency_maps.numpy()

        # np.save('saliency_maps.npy', saliency_maps_np)
        # print("Saliency maps saved to saliency_maps.npy")
